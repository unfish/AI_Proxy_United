...
[ApiClass(M.语音服务, "语音服务", "提供输入文字转语音服务，和输入语音转文字服务。多种接口和音色可选。", 300, type: ApiClassTypeEnum.辅助模型, canProcessAudio:true)]
public class ApiAudio:ApiBase
{
    ...
    {
        var qc = input.QuestionContents.FirstOrDefault();
    ...
    protected override async Task<Result> DoProcessQuery(ApiChatInputIntern input)
    {
        ...
        if (qc.Type == ChatType.文本)
        {
            var resp = await _audioService.TextToVoice(qc.Content, input.AudioVoice, input.AudioFormat); //发送文字转语音
            return resp;
        }
        else if (qc.Type == ChatType.文件Bytes)
        {
