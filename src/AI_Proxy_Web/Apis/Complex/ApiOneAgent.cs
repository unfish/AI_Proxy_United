...
[ApiClass(M.OneAgent, "万能助理", "提出一个复杂任务，由模型进行任务分解，并自动进行任务编排，进行多模型调度，合作共同完成复杂任务，比如市场调研+写方案，客户需求分析+方案+代码，调用浏览器查询资料并生成文件导出数据等等。更多功能有待挖掘。", 197, type: ApiClassTypeEnum.辅助模型, canProcessImage:true,canProcessFile:true, needLongProcessTime:true, priceIn: 0, priceOut: 0.1)]
    ...
    public async IAsyncEnumerable<Result> SendMessageStream(ApiChatInputIntern input)
    {
        ...
        if (isFirstChat) //首次进入增加系统指令
        {
            var question = "当你接收到用户的需求，请认真分析用户的目的及深层需求，并列出所有该任务需要用户明确的需求点，例如调研的方向、研究范围、边界、明确的目标市场或目标客户群等等，等用户回答完以后再开始解决问题。先向用户展示你准备采取的工作步骤，然后自动开始执行。\n" +
                           ...
                           "当完成拆解任务后，调用万能助理的文件助手功能将任务步骤写入todo.md文件。然后按步骤执行，每一步分别调用合适的助理来进行，比如调用信息搜集助手搜索和收集互联网信息，调用方案设计助手完成客户需要的新方案的编写等等。" +
                           "在每一步助理完成并返回结果以后，都要调用一次文件助手将上一步的任务结果更新到todo.md文件里对应的位置。如果这一步任务的结果属于最终报告的一部分，调用追加文件方法将任务的完整结果追加到report.md文件中。完整结果不用重复输出到任务描述中，通过need_contexts参数传递前一步任务的role名称即可。" +
                           "将所有任务完成以后，再调用文件助手将todo.md和report.md两个文件发给用户。";
